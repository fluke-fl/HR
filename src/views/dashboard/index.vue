<template>
  <div class="dashboard-container">
    ...
    <!-- 离职弹层 -->
    <el-dialog :visible="visible" title="离职申请" @close="btnCancel">
      <el-form
        ref="ruleForm"
        label-width="120px"
        status-icon
...
import workRadar from './components/radar.vue'
import { startProcess } from '@/api/approvals'

export default {
  name: 'Dashboard',
  components: { workCalender, workRadar },
  data() {
    return {
      ...
      rules: {
        exceptTime: [{ trigger: 'blur', required: true, message: '不能为空' }],
        reason: [{ trigger: 'blur', required: true, message: '不能为空' }]
      }
    }
  },
  computed: {
