...
<script>
import { getPersonInfo, getPersonJob } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      this.formData = await getPersonInfo(this.userId) // 获取个人基本信息
    },
    async getJobDetail() {
      const userInfo = await getUserDetailById(this.userId)
      const jobInfo = await getPersonJob(this.userId) // 获取个人基本信息
