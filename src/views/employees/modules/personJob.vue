...
import { getPersonJob, getUserSimple, putPersonJob } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    async getPersonJob() {
      const res = await getPersonJob(this.userId)
      // console.log(res)
      const res1 = await getUserSimple()
      // console.log(res1)
      this.depts = res1
      this.formData = res
    },
    async saveJob() {
      await putPersonJob(this.formData)
      this.$message('保存成功')
    }
  }
}
